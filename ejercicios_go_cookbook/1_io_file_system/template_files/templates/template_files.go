...
//CreateTemplate creará un archivo de plantilla que contiene datos
func CreateTemplate(path string, data string) error {
	return ioutil.WriteFile(path, []byte(data), os.FileMode(0755))
}

// InitTemplates configura plantillas desde un directorio
func InitTemplates() error {
	...
	// Parse glob combinará todos los archivos que coincidan con glob
	// y combinarlos en una sola plantilla
	tmpl, err := template.ParseGlob(pattern)
	if err != nil {
		return err
	}

	// Execute también puede trabajar con un mapa
	// de una estructura
