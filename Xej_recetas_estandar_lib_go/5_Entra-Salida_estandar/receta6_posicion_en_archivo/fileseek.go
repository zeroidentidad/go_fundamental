...
func main() {
	...
	if err := writeRecord(2, "first", "Zero", f); err != nil {
		panic(err)
	}
	fmt.Println(readRecords(2, "first", f))
	if err := writeRecord(10, "first", "Identidad", f); err != nil {
		panic(err)
	}
...
func writeRecord(line int, column, dataStr string, f *os.File) error {
	...
	case "last":
		position += 14
	default:
		return errors.New("Columna no definida")
	}

	if len([]byte(dataStr)) > definedLen {
		return fmt.Errorf("Longitud máxima para '%s' es %d", column, definedLen)
	}
...
func readRecords(line int, column string, f *os.File) (string, error) {
	...
	return retVal, errors.New("Columna no definida")
}
